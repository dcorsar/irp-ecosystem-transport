...
public class TimeTableResource implements RESTFulSPARQL
{
  ...
  public Lines getLinesInAdminArea(@PathParam("Line") String line,
                                    ...
                                    @DefaultValue("false") @QueryParam("includeDirections") boolean includeDirections)
  {
    ...
    while(results.hasNext())
    {
      ...
      Line route = uk.ac.dotrural.irp.ecosystem.transport.Util.contains(lines, currentLineURI);
                                      ...
                                      @DefaultValue("inbound") @QueryParam("direction") String direction)
  {
    ...
    while(busLocationsOnRoute.hasNext())
    {
      ...
      location.setEasting(Double.parseDouble(Util.getNodeValue(solution.get(vars.get(3))).trim()));
      location.setNorthing(Double.parseDouble(Util.getNodeValue(solution.get(vars.get(4))).trim()));
      location.setLongitude(Double.parseDouble(Util.getNodeValue(solution.get(vars.get(5))).trim()));
      location.setLatitude(Double.parseDouble(Util.getNodeValue(solution.get(vars.get(6))).trim()));
